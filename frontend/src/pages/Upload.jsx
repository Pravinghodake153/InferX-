    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          <WorkflowStep 
            done={tenderUploaded || project?.extractionStatus === 'complete' || project?.status === 'reviewed' || project?.status === 'evaluated'} 
            label="Data Ingested" 
          />
          <WorkflowStep
            done={project?.extractionStatus === 'complete' || project?.status === 'reviewed' || project?.status === 'evaluated'}
            pending={extracting || project?.extractionStatus === 'running'}
            label="Extraction Complete"
          />
          <WorkflowStep 
            done={(project?.reviewData?.manualMasks && Object.keys(project.reviewData.manualMasks).length > 0) || project?.status === 'reviewed' || project?.status === 'evaluated'}
            pending={project?.extractionStatus === 'complete' && (!project?.reviewData?.manualMasks || Object.keys(project.reviewData.manualMasks || {}).length === 0) && project?.status !== 'reviewed' && project?.status !== 'evaluated'}
            label="PII Masked" 
          />
          <WorkflowStep
            done={project?.criteriaLocked || project?.status === 'reviewed' || project?.status === 'evaluated'}
            pending={project?.extractionStatus === 'complete' && !project?.criteriaLocked && project?.status !== 'reviewed' && project?.status !== 'evaluated'}
